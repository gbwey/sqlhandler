...
allTests =
  ...
  , expectD (Right ((unsafeRefined3 [127,1,0,199] "127.001.000.199") ,[])) (unDec (defDec :: Dec (Refined3 (Map (ReadP Int) (Resplit "\\." Id)) (Guard "length" (Len >> Same 4) >> Guard "octet 0-255" (All (Between 0 255)) >> 'True) (Printfnt 4 "%03d.%03d.%03d.%03d") String)) [SqlString "127.1.0.199"])
  , expectD (Left "Refined3 Step 2. Failed Boolean Check(op) | octet 0-255") (unDec (defDec :: Dec (Refined3 (Map (ReadP Int) (Resplit "\\." Id)) (Guard "length" (Len >> Same 4) >> Guard "octet 0-255" (All (Between 0 255)) >> 'True) (Printfnt 4 "%03d.%03d.%03d.%03d") String)) [SqlString "127.1.0.499"])
